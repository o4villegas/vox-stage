...
| Mic capture | `getUserMedia` with ALL processing OFF (echoCancellation included) — **decided by device A/B (R41/R42)**: requesting EC on iOS collapses the mic pipeline (46.9→29.2 windows/s, 11.4 s clock stall) via the voice-processing route. Matches and extends Rangefinder's guardrail. Consequence: scoring tolerates backing-track bleed via clarity gating + singer level dominance; headphones-recommended mode as the high-accuracy option **[bleed robustness validated in M5]** | R41, R42, R24 |
